...
module.exports = bot => {
    ...
    const xmas = schedule.scheduleJob({month:10, date:31}, function(){
        bot.user.setActivity(`Merry Christmas nya!`, {type: 'WATCHING'}).catch(console.error)
    });

    //Floating dates
    const easter = schedule.scheduleJob({month:4, date: 4}, function(){
        bot.user.setActivity(`Happy Easter nya~!`, {type: 'WATCHING'}).catch(console.error)
    });
    const mother = schedule.scheduleJob({month:5, date:2}, function(){
        bot.user.setActivity(`Happy Mother's Day nya~!`, {type: 'WATCHING'}).catch(console.error)
    });
    const father = schedule.scheduleJob({month:6, date:20}, function(){
